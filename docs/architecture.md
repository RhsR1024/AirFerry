...
> **持续修复与长播退化策略**：源符号发完后优先产生从未见过的 Repair；新鲜 Repair ESI 接近 2²⁴ 时固定该游标，并由独立游标显式循环重放合法 Repair 命名空间，避免回绕、panic、对象停播以及固定四码位捕获只能看到 1/4 Source 的永久不收敛。`redundancy_pct` 同时参与窗口预算。
...
> **并行解码池 / 解码摄入分离**：采集与解码解耦；QR 识别可并行，但原生 receiver 句柄非线程安全，ingest 必须串行。完成时先停止摄入再 `assemble()`；会话切换和进度快照都在 ingest 锁内完成。Android、Windows、Web 共享“区域热路径 + miss/固定周期全帧发现”策略；全帧保留 TryInvert，已锁定区域跳过反色重试。进度 UI 从一致快照展示去重后的接收进度、3 秒窗口速率和有效吞吐。
