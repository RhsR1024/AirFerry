...
> 🤖 **AI 代理/新开发者**：先读 [AGENTS.md](../AGENTS.md)（构建命令、代码导航、调试速查、文档与代码偏差清单）。跨端线格式的位级权威定义见 [docs/SPEC.md](SPEC.md)。
...
| Windows App | `./scripts/build-windows.ps1` | 须 Windows + .NET 8 SDK + CMake/VS C++（详见 [docs/build-windows.md](build-windows.md)） |
...
- [协议规范](protocol.md) — 完整协议描述
- [跨端契约规格](SPEC.md) — 线格式/会话 ID/JNI 位布局等位级权威定义
- [二维码帧格式](qr-frame-format.md) — 帧头字段定义
- [RaptorQ 参数](raptorq-params.md) — 编解码参数说明
- [架构设计](architecture.md) — 系统架构与组件关系
- [数据流](data-flow.md) — 端到端数据流详解
- [API 参考](api.md) — 核心 API 文档
- [构建指南 - 浏览器扩展](build-browser.md)
- [构建指南 - Android](build-android.md)
- [构建指南 - Windows](build-windows.md)
- [开发环境搭建](dev-setup.md)
