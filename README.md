...
> ⚠️ **安全边界**：AF2 v2 校验传输损坏与混流，但不认证发送者、也不加密二维码内容。敏感文件请先使用带认证的加密工具封装，并通过独立可信渠道核对摘要/Content ID；不要在攻击者可覆盖摄像头画面的环境中把“校验通过”当作来源可信。
...
- ✅ 持续喷泉码：源符号发一遍后持续补充不重复修复符号；到 RFC 24 位 ESI 上限后显式循环重放合法 Repair 命名空间，游标不回绕、对象不停播
